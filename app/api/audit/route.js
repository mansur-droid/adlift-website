...
const buckets=globalThis.__adliftAuditBuckets||(globalThis.__adliftAuditBuckets=new Map());
function limited(ip){const now=Date.now(),windowMs=10*60*1000,limit=5;const hits=(buckets.get(ip)||[]).filter(t=>now-t<windowMs);hits.push(now);buckets.set(ip,hits);return hits.length>limit;}

export async function POST(request){
  try{
    const ip=(request.headers.get('x-forwarded-for')||'unknown').split(',')[0].trim();
    if(limited(ip)) return NextResponse.json({error:'Too many audit requests. Please try again in a few minutes.'},{status:429});
    const body=await request.json();
    if(body.website) return NextResponse.json({ok:true});
    const row={
      first_name:clean(body.first_name,80), email:clean(body.email,200).toLowerCase(), phone:clean(body.phone,40)||null,
      brokerage:clean(body.brokerage,150)||null, city:clean(body.city,100), state:clean(body.state,100), average_home_price:clean(body.average_home_price,100),
      target_buyer_type:cleanList(body.target_buyer_type), target_price_range:clean(body.target_price_range,100), target_areas:clean(body.target_areas,MAX.long),
